    ...
    public class ParticleWetSplatter : MonoBehaviour
    {
        ...
        protected virtual void Update()
        {
            ...
            for (var i = 0; i < _collisionEvents.Count; i++)
            {
                ...
                if (Limit.Enabled)
                {
                    chance *= Limit.DecalChance.Evaluate((float) _totalDecalCount / Limit.MaxDecals);
        ...
        private Splatter GetOrCreateSplatter()
        {
            ...
            // Try to steal an active decal if we can
            if (_recycling.Enabled && _recycling.StealThreshold > 0)
            {
                // Find the first decal which we can steal. We'll assume the oldest decals are the the start so only search a maximum of 25 items
            ...
            // If limit is enabled and we've hit it do nothing
